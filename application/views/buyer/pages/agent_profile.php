...
<div class="card">
    <div class="card-header header-elements-inline">
        ...
        <?php if (!$agent_licenses) { ?>
            <div class="ribbon ribbon-top-right ribbonred"><span>Inactive</span></div><?php } ?>
    </div>
    <div class="card-body">
        <div class="row">
            <div class="col-md-12 py-3 px-lg-3 px-xl-3 dark-bg profile-header">
                ...
                <h6 class=""><?php echo $agent_account['brokerage_name']; ?></h6>
                <div class="messagebutton">
                    <?php
                    if (isset($agent_chat) && isset($agent_chat['feature_id'])) {
                        ?>
                        <a href="<?php echo cortiam_base_url('edit-account'); ?>"
                           class="button-border-white smallerbutton sendmessagebutton">Send Message</a>
                        <?php
                ...
                <?php
                $data = activeChatFeatures($agent_id, 'agent_intro_video');
                $chat = activeChatFeatures($agent_id, 'chat');

                ?>

                <div class="text-right mt-4" style="margin-top: 90px !important;">
                    <div class="btn-group" role="group" aria-label="Basic example">
                        <?php
                        if (!isset($agent_selected['id']) || empty($agent_selected['id'])) {
                            ?>
                            <a href="javascript:void(0);" class="button-orange sendreview mr-2 rounded"
                               data-agent-id="<?= $agent_id ?>" data-buyer-id="<?= $buyer_id ?>">Select this agent</a>
                            <a href="javascript:void(0);" class="button-orange deleletereview mr-2 rounded"
                               style="display:none;" data-agent-id="<?= $agent_id ?>" data-buyer-id="<?= $buyer_id ?>">Unselect
                                this agent</a>
                            ...
                            <a href="javascript:void(0);" class="button-orange sendreview mr-2 rounded"
                               style="display:none;" data-agent-id="<?= $agent_id ?>" data-buyer-id="<?= $buyer_id ?>">Select
                                this agent</a>
                            <a href="javascript:void(0);" class="button-orange deleletereview mr-2 rounded "
                               data-agent-id="<?= $agent_id ?>" data-buyer-id="<?= $buyer_id ?>">Unselect this agent</a>

                            <?php

                        }
                        if (isset($chat['id']) || !empty($chat['id'])) {
                            ?>
                            <a href="#!" class="button-orange mr-2 rounded" data-toggle="modal"
                               data-target="#sendMultipleMessageToAgent">Send Message</a>
            ...
            ?>
                <form method="POST" class="bookslotajaxform w-100" data-source="formajaxurl" autocomplete="off"
                      id="bookslotajaxform">
                            ...
                            <fieldset>
                                <div class="row mt-3">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <div class="form-group">
                                            <select class="form-control available-slots" id="available-slots"
                                                    name="available-slots" required>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        ...
                                        <div class="form-group">
                                            <select class="form-control available-slots" id="meet-type" name="meet-type"
                                                    required>
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-3"><img class="img-fluid"
                                               src="<?php echo base_url($agent_proposal['default_image']); ?>"></div>
                    <div class="col-md-9">
            ...
            <div class="card-footer">
                <?php if (in_array($agent_proposal['status'], array('Unread', 'Read'))) { ?>
                    <div class="col-md-12 mt-1 px-lg-3 px-xl-3 text-center buttonsrow">
                        <button class="button-border-orange smallerbutton text-center mr-3 acceptproposal"
                                data-prop="<?php echo $agent_proposal['prop_id']; ?>">ACCEPT THIS OFFER
                        </button>
                        <button class="button-border-dark smallerbutton text-center mr-3 declineproposal"
                                data-prop="<?php echo $agent_proposal['prop_id']; ?>">DECLINE THIS OFFER
                        </button>
                        <button class="button-border-gray smallerbutton text-center counterofferproposal"
                                data-prop="<?php echo $agent_proposal['prop_id']; ?>">COUNTER OFFER
                        </button>
                    </div>
                <?php } ?>
            </div>
...
<div class="modal fade" id="sendMultipleMessageToAgent" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog modal-lg" role="document">
        <form action="<?= base_url('buyer/ajax/send-message-from-buyer') ?>"
              method="post" id="">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="hidden" name="message_from_buyer_dashboard" value="1">
                        <input type="hidden" name="agent_id" value="<?= $agent_account['agent_id'] ?>"/>
                        <input type="hidden" name="fromAgentProfile" value="fromAgentProfile"/>

                        <input type="email" id="email" name="email" value="<?= $agent_account['email'] ?>" readonly/>
...
<!-- The Modal -->
<div class="modal" id="contactModalBox">
    <div class="modal-dialog">
        <div class="modal-content">

            <!-- Modal Header -->
            <form id="contactnotification">

            <div class="modal-header">
                <h5 style="color:#002C77; font-weight: bold;">Congrats on finding your Agent. Please fill out this form so the agent can reach out to you. Your info is only shared with this agent. Also, you may reach out to them directly.</h5>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>
            <input type="hidden" id="contact_agent_id" name="agent_id" value="" />
            <!-- Modal body -->
            <div class="modal-body">
                <div class="form-group">
                    <input type="text" class="form-control" id="buyername" name="buyername" placeholder="Buyer Name" value="<?= (isset($notificationInfo['name']) )? $notificationInfo['name'] : $this->account['first_name'] .' '. $this->account['last_name']; ?>" required />
                </div>
                <div class="form-group">
                    <input type="text" class="form-control" id="notitication_phone" name="phone" placeholder="#Phone" value="<?= (isset($notificationInfo['phone']) )? $notificationInfo['phone'] : $this->account['phone']; ?>" required>
                </div>
                <div class="form-group">
                    <input type="text" class="form-control" id="buyeremail" name="buyeremail" placeholder="Email" value="<?= (isset($notificationInfo['email']) )? $notificationInfo['email'] : $this->account['email']; ?>" required>
                </div>

                <div class="col-sm-12">
                    <div class="row">
                        <span>By submitting this form, you agree to our <a href="<?= base_url('/terms-of-use') ?>" style="color:#0a6ebd;">Terms and Conditions</a></span>
                    </div>
                </div>
            </div>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-secondary" data-dismiss="modal" id="willcontactagain">I'll contact Agent</button>
            </div>
