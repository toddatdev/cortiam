...
<script>
	...
	// jQuery('.getstate').select2({
	// 	  data: oldStates,
	// 	placeholder: 'Select a State',
	// 	allowClear: true
	// });
	// jQuery('.getcity').select2({
	// 	  data: oldStates,
	// 	placeholder: 'Select a City',
	// 	allowClear: true
	// });
	...
	jQuery('#getstate').on('change', function (e) {
        jQuery('#getcity').html('');
        ...
        if(window.location.origin == 'http://localhost') {
            getUrl = "/cortiam/buyer/ajax/get-all-cities";
        }else if(window.location.origin == 'http://192.168.104.85'){
              getUrl = "/cortiam/buyer/ajax/get-all-cities";
        }else{
            getUrl = "/buyer/ajax/get-all-cities";
        }
        $.ajax({
            url: getUrl,
            type: "POST",
            ...
            success: function (dataResult) {
                ...
                if(dataResult)
                {
                    ...
                    for (let index = 0; index <dataResult.length; index++)
                    {
                        ...
                        $('#getcity').append(newOption).trigger('change');

                    }

                    $('#getcity').val('').trigger('change');
                    // $('#getcity').val($('#getcity option:first-child').val()).trigger('change');
	...
	$(".setStateEmpty").val('').trigger('change');
	$(".setCityEmpty").val('').trigger('change');
