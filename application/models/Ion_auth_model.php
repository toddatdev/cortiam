...
class Ion_auth_model extends CI_Model
{
    ...
    public function register($identity, $password, $email, $additional_data = [] )
    {
        ...
        $user_email_text = '<a href="'.base_url('verify/email').'/' . $additional_data['email_verification_token']. '" style="border-radius:5px;padding:10px 30px;background:#00c48d;color:#ffffff;text-align:center;font-weight:bold;text-transform:uppercase;margin:10px 0px;display:inline-block;text-decoration:none;" target="_blank" class="loginlink">Verify Email</a></p>';
     ...
     * @author    Mathew
     */
    public function login($identity, $password, $remember = FALSE, $user_type)
    {     
        ...
        $query =    $query->where($this->identity_column, $identity);            
//        $query = $query->where('email_verified_at !=', null)
        ...
        if ($query->num_rows() === 1) {
            ...
            $getUserType = $query->row();                
         

            if(isset($user_type) && !empty($user_type))
            {
                // if($user_type !== $getUserType->user_type)
                // {
                //     $this->trigger_events('post_login_unsuccessful');
                ...
                if($user_type == 'Seller' || $user_type == 'Buyer')
                {
                    ...
                    //$this->db->update('accounts', $data, "email = '".$identity."'");
                    ...
                    if((isset($getUser->id) && !empty($getUser->id)) && ($getUser->user_type !== 'Administrator' || $getUser->user_type !== 'Agent'))
                    {
                        ...
                        if($user_type == 'Seller')
                        {
                            ...
                            $data = array('status' => 0);
                            $this->db->where('buyer_id', $getUser->id);
                            ...
                            {                       
                                ...
                                if(isset($getBuyerUser->first_name) && $getBuyerUser->first_name !== '')
                                {
                              
                                   
                                    $this->db->where('seller_id', $getUser->id);
                            ...
                            if(!isset($getbuyerUSer->buyer_id) && empty($getbuyerUSer->buyer_id))
                            {
                                ...
                                if(isset($getSellerUser->first_name) && $getSellerUser->first_name !== '')
                                {
                                    $this->db->where('buyer_id', $getUser->id);
                                    $this->db->update('buyers', $data);
                                }
            ...
            if($user->user_type == 'Administrator' || $user->user_type == 'Agent')
            {
    ...
    {
        $query = $this->db->select('email_verification_token, email')
        ...
        if ($query->num_rows() === 1) {
          ...
          $user_email_text = '<h3 style="color:#4c525e;">WELCOME TO CORTIAM</h3><h4 style="color:#848994;">Thank You for Joining Cortiam!</h4><p>Your registration has been submitted successfully. Your account is activated and ready for use. You can login to your account with the email and password you selected and complete setting up your profile or add your property for listing.</p><p style="text-align:center;"><a href="'.base_url('login').'" style="border-radius:5px;padding:10px 30px;background:#00c48d;color:#ffffff;text-align:center;text-transform:uppercase;font-weight:bold;margin:10px 0px;display:inline-block;text-decoration:none;" target="_blank" class="loginlink">Login to Cortiam</a></p>';
          $this->mailer->regular_email('Thank You for Joining Cortiam!', $user_email_text,  $query->row()->email);    
