		...
		if ($this->input->post("proposal_id")) {
			...
			if(count($response["errorfields"])) {
				...
				if($new_proposal_id = $this->seller_model->add_proposal($msg_data)){
					...
					try {
						$user_email_text = '<h3 style="color:#4c525e;">NEW COUNTER OFFER</h3><h4 style="color:#848994;">Dear '.$agent_account['first_name'].' '.$agent_account['last_name'].'</h4><p>We are happy to tell you that the property owner at '.$proposal['city'].' '.$proposal['state'].' sent you a new counter offer. For more information please login to Cortiam to check this offer details.</p><p style="text-align:center;"><a href="'.base_url('login').'" style="border-radius:5px;padding:10px 30px;background:#00c48d;color:#ffffff;text-align:center;text-transform:uppercase;font-weight:bold;margin:10px 0px;display:inline-block;text-decoration:none;" target="_blank" class="loginlink">Login to Cortiam</a></p>';
			...
			if(!$proposal = $this->seller_model->get_proposal($this->ion_auth->get_user_id(), $this->input->post("proposal_id"))){
				...
				if($agreement_check = $this->seller_model->check_for_agreement($proposal['property_id'])){
					...
					if($this->seller_model->edit_proposal($proposal['prop_id'],$msg_data)){
                        ...
                        $agentCoupon = $this->seller_model->get_coupon_discount($proposal['agent_id']);
...
//							'agr_fee' => $fee_amount,
						...
						$userEmail = $this->seller_model->get_seller_info( $proposal['seller_id']);
						$seller    = $this->seller_model->getSellerName($proposal['seller_id']);						 
						...
						$this->seller_model->saveReviewForm($data);
						...
						try {
							...
							$this->seller_model->add_notification($proposal['agent_id'], 'Your Offer Accepted', 'We are happy to tell you that property owner of property at '.$proposal['city'].', '.$proposal['state'].' accepted your offer. We have created an agreement for your final review');
			...
			if(!$proposal = $this->seller_model->get_proposal($this->ion_auth->get_user_id(), $this->input->post("proposal_id"))){
				...
				if($this->seller_model->edit_proposal($proposal['prop_id'],$msg_data)){
					...
					try {
						...
						$this->seller_model->add_notification($proposal['agent_id'], 'Your Offer Declined', 'We are sorry to tell you that the property owner of the property at .' .$proposal['city'].', '.$proposal['state'].' has declined your offer');
					} catch (\Throwable $th) {
						//throw $th;
					}									
	...
	public function send_message(){
		...
		if ($this->input->post("message_from_seller_dashboard") == 1){

		    redirect(base_url('seller/message-center'));
	...
	public function send_message_to($agent, $agent_id){
		if ($agent = $this->seller_model->get_agent($agent_id)){
			...
			if( isset($response["errorfields"]) && count($response["errorfields"])) {
				$response["fail"] = true;
				$response["fail_title"] = 'Required/Missing Fields!';
				$response["fail_message"] = implode(', ',$response["errorfields"]). ' field(s) are required. Please fill out all the required/missing fields and try again.';
			}else{
		...
		if ($this->input->post("agree_id")) {
			if(!$agreement = $this->seller_model->get_aggrement($this->ion_auth->get_user_id(), $this->input->post("agree_id"))){
				...
				if($this->seller_model->edit_agreement($agreement['agr_id'],$agr_data)){
					...
					try {
						$user_email_text = '<h3 style="color:#4c525e;">AGREEMENT ACCEPTED</h3><h4 style="color:#848994;">Dear '.$agent_account['first_name'].' '.$agent_account['last_name'].'</h4><p>We are happy to tell you that the property owner accepted the agreement. For more information please login to Cortiam to check the agreement details.</p><p style="text-align:center;"><a href="'.base_url('login').'" style="border-radius:5px;padding:10px 30px;background:#00c48d;color:#ffffff;text-align:center;text-transform:uppercase;font-weight:bold;margin:10px 0px;display:inline-block;text-decoration:none;" target="_blank" class="loginlink">Login to Cortiam</a></p>';
						if ($agent_account['notifications'] == 'Yes') {
    ...
    public function get_agent_slots()
    {
        ...
        $agent_id = $this->input->post("agent_id");
        $data = $this->seller_model->get_agent_slots($day, $agent_id);
    ...
    public function book_available_agent_slot()
    {
        ...
        $agent_id = $this->input->post("agent_id");
        $seller_id = $this->ion_auth->get_user_id();
        $data = $this->seller_model->book_available_agent_slot($_REQUEST, $agent_id, $seller_id);
        if(isset($data['type'])  &&  $data['type'] == 'fail')
        {
            echo json_encode($data);
            exit;
        }
        $agent_account = $this->seller_model->get_agent($agent_id);
        $seller_account = $this->seller_model->get_seller($seller_id);
//        $booked_Days_array   = $this->seller_model->get_booked_Days($agent_id);
...
//                    $this->page_data['bookedAppointments'] = NULL;
        ...
        try {
            ...
            {
                $user_email_text = '<h3 style="color:#4c525e;">NEW APPOINTMENT</h3><h4 style="color:#848994;">Dear ' . $agent_account['first_name'] . ' ' . $agent_account['last_name'] . ',</h4><p>An appointment has been requested!  The '. $seller_account['first_name'].' '.$seller_account['last_name'].' would like to meet over video chat. Please send them your video link in this chat. This appointment is for ' .$_REQUEST['select-date']. ' and '.$_REQUEST['available-slots'].'</p><p style="text-align:center;"><a href="' . base_url('login') . '" style="border-radius:5px;padding:10px 30px;background:#00c48d;color:#ffffff;text-align:center;text-transform:uppercase;font-weight:bold;margin:10px 0px;display:inline-block;text-decoration:none;" target="_blank" class="loginlink">Login to Cortiam</a></p>';
                $this->agent_model->add_notification($agent_id , 'New appointment', 'The '. $seller_account['first_name'].' '.$seller_account['last_name'].'  would like to schedule a video chat appointment for ' .$_REQUEST['select-date']. ' .You can send the video chat link to this person at '.$seller_account['email']);
                $this->mailer->regular_email('New appointment! The '. $seller_account['first_name'].' '.$seller_account['last_name'].' would like to schedule a video chat appointment for '.$_REQUEST['select-date'], $user_email_text, $agent_account['email']);
            }elseif( $_REQUEST['meet-type']  == 'phone'){
                $user_email_text = '<h3 style="color:#4c525e;">NEW APPOINTMENT</h3><h4 style="color:#848994;">Dear ' . $agent_account['first_name'] . ' ' . $agent_account['last_name'] . ',</h4><p>An appointment has been requested! Please contact the '. $seller_account['first_name'].' '.$seller_account['last_name'].' at '.$seller_account['phone'].',  This appointment is for ' .$_REQUEST['select-date']. ' at '.$_REQUEST['available-slots'].'</p><p style="text-align:center;"><a href="' . base_url('login') . '" style="border-radius:5px;padding:10px 30px;background:#00c48d;color:#ffffff;text-align:center;text-transform:uppercase;font-weight:bold;margin:10px 0px;display:inline-block;text-decoration:none;" target="_blank" class="loginlink">Login to Cortiam</a></p>';
                $this->agent_model->add_notification($agent_id , 'New appointment', 'The '. $seller_account['first_name'].' '.$seller_account['last_name'].' would like to schedule a phone call appointment for  '.$seller_account['phone'] );
                $this->mailer->regular_email('New appointment! The '. $seller_account['first_name'].' '.$seller_account['last_name'].' would like to schedule a phone call appointment for '.$seller_account['phone'], $user_email_text, $agent_account['email']);
            }
        ...
        $dataArray = json_encode(['bookeddays' => $bookdaystring, 'allowedMonths' => implode(",",$this->page_data['monthArray']), 'allowedYears' => implode(",",$this->page_data['yearArray']), 'successsvalues' => $data]);
        echo $dataArray;
    ...
    {
	    $agent = $this->seller_model->get_express_list($this->ion_auth->get_user_id(),true);
        $agent_id = $agent[0];
        $agent_name = $this->seller_model->get_agent_name($agent_id);
        ...
        if ($records = $this->seller_model->get_agent_appointments($_REQUEST, $this->ion_auth->get_user_id()))
        {
            foreach ($records as $key => $record)
            {
                ...
                $data[] = array(
                    ...
                    "message"     => '<a a href="'.base_url('seller/view-messages/Agent/').$record['agent_id'].'" title="Message"><span class="icon-co-sm envelope"></span></a>'
                );
            }
        }

        $totalRecordwithFilter = count($records);
        $totalRecords = $this->seller_model->get_total_agent_appointments($this->ion_auth->get_user_id(), $search);
        $response = array(
            "draw" => intval($draw),
	...
	public function attribute_submit()
	{
		...
		{	
			$this->db->delete('users_attributes', array('user_id' => $account_id, 'user_type' => 'Seller'));
	...
	{
		if ($dealed_agents = $this->seller_model->get_express_list($this->ion_auth->get_user_id(), true)) {
            $saved_agents = $this->seller_model->list_agents(array('seller_id' => $this->ion_auth->get_user_id(), 'list' => $dealed_agents, 'limit' => 15));
			$list = array();
			foreach ($saved_agents as $key => $agent)
			{
				$list[] = new_card($agent, "seller", $this->ion_auth->get_user_id());
			}
	...
	public function get_favorite_agent()
	{
        if ($saved_agents = $this->seller_model->get_favorite_list($this->ion_auth->get_user_id(), true)) {
			...
			$list = array();
			foreach ($this->page_data['saved_agents'] as $key => $agent)
			{
				$list[] = new_card($agent, "seller", $this->ion_auth->get_user_id());
			}
	...
	public function get_all_cities()
	{
		...
		$cities   = $this->seller_model->getAllCities($stateName);
    ...
    {
       $seller_id = $this->ion_auth->get_user_id();
        $notification = $this->seller_model->setNotification($_REQUEST, $seller_id);
    ...
    public function buyer_agent_rating_list()
    {
        ...
        if ($records = $this->seller_model->get_seller_agents_rating($_REQUEST)) {



            foreach ($records as $key => $record) {
                $id = $record["id"];
                $agent_email = getAgentEmail($record['seller_id']);

                $agent_id = $record["agent_firstName"] . " " . $record["seller_lastName"] . " <br> " . $agent_email;
                ...
                $action  = '<center><div class="row"></div><a href="javascript:void(0);" class="button-orange smallerbutton float-left delete" data-delete="' . $record["id"] . '">Delete</a>';
    ...
    public function seller_agent_rating_list()
    {
        $data = array();
        $draw = $_REQUEST['draw'];
    ...
    public function seller_rating_details_list()
    {
        $records = $this->seller_model->seller_rating_details_list($_REQUEST['reviewid']);
    ...
    {
        if ($records = $this->seller_model->delete_agent_rating($_REQUEST)) {
            echo "success";
        } else {
            echo "fail";
        }
