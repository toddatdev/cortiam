	...
	jQuery( "form.signupform" ).validate({
		...
		submitHandler: function(form, event) {
			...
			if (!$('#last_name').val())
			{
				...
				{
					$("#last_name").parents('.l-name').after("<div class='from-validation' style='color:red;margin-bottom: 20px;text-align: start'>Please last name</div>");
				}
			}
			else {
			...
			if($( "#invalidSignerCheck" ).prop( "checked") == false)
			{
				if ($("#invalidSignerCheck").parents('.termsandcondition').next(".from-validation").length == 0) // only add if not added
				{
					$("#invalidSignerCheck").parents('.termsandcondition').after("<div class='from-validation' style='color:red;margin-bottom: 20px;text-align: start'>Please! accept terms & Condtion</div>");
					return false;
					...
					$("#invalidSignerCheck").parents('.termsandcondition').next(".from-validation").remove(); // remove it
					jQuery('#response').html('');
				}
			}
