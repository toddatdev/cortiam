...
$(document).ready(function() {
	...
	$('body').on( "click", '.favoritebutton', function(ev) {
		ev.preventDefault();
		favbutton = $(this);
		favbutton_value = $(favbutton).data('type');
		jQuery.ajax({
			type: "post",
			...
			success: function(response){
				if(response.buttonicon){
					$(favbutton).html(response.buttonicon);
				}
				if(response.buttonvalue){
					$(favbutton).data('type', response.buttonvalue);
				}
				if(response.success){
					swal.fire({
	...
	$('body').on( "click", '.sendmessagebutton', function(ev) {
		ev.preventDefault();
		button = $(this);
		jQuery.ajax({
			type: "post",
	...
	$('body').on( "click", '#send-message-now', function(ev) {
		ev.preventDefault();
		button = $(this);
		var form_data = $('#offer-form').serializeArray();
		...
		jQuery.ajax({
			...
			dataType: "json",
			beforeSend: function() {
				$('#offer-form').block({message: '<img src="' + cortiamajax.loadingimage + '"></div>',css: {border:'0px',width:'100%',top:'10%',left:'0%',background:'transparent', cursor:'context-menu'},overlayCSS: {backgroundColor:'#ffffff',opacity:.9}});
			},
			success: function(response){
				if(response.success){
					$('.buttonsrow').remove();
				...
				if(response.errorfields){
					jQuery.each(response.errorfields, function(index, value) {
						if($("#"+index).hasClass('select2-hidden-accessible')){
							$("#"+index).next('span.select2-container').find('.select2-selection--single').addClass("border-danger").one("focus", function() {
								$(this).removeClass("border-danger");
							});
						}else{
							$("#"+index).addClass("border-danger").one("focus", function() {
								$(this).removeClass("border-danger");
							});
						}
					});
	...
	$('body').on( "click", '#cancel-message-now', function(ev) {
		ev.preventDefault();
		jQuery.unblockUI();
	});


	$('body').on( "click", '.acceptproposal', function(ev) {
		ev.preventDefault();
		proposal_id = $(this).data('prop');
		swal.fire({
			title: "Are you sure?",
			...
			if(e.value){
				jQuery.ajax({
					...
					success: function(response){
						if(response.success){
							$('.buttonsrow, .messagebutton').remove();
							swal.fire({
								title: response.success_title,
								text: response.success_message,
								type: "success",
	...
	$('body').on( "click", '.declineproposal', function(ev) {
		ev.preventDefault();
		proposal_id = $(this).data('prop');
		swal.fire({
			title: "Are you sure?",
	...
	$('body').on( "click", '.counterofferproposal', function(ev) {
		ev.preventDefault();
		proposal_id = $(this).data('prop');
		button = $(this);
		jQuery.ajax({
	...
	$('body').on( "click", '.withdrawproposal', function(ev) {
		ev.preventDefault();
		button = $(this);
		proposal_id = $(this).data('prop');
		swal.fire({
				...
				jQuery.ajax({
					...
					success: function(response){
						if(response.success){
							...
							$('#couponlistpart .ribbon').replaceWith('<div class="ribbon ribbon-top-right ribbonred"><span>Withdrawn</span></div>');
						}
						if(response.fail){
							jQuery.unblockUI();
							swal.fire({
	...
	$('body').on( "click", '#send-counter-offer', function(ev) {
		ev.preventDefault();
		button = $(this);
		proposal_id = $(this).data('prop');
		var form_data = $('#offer-form').serializeArray();
	...
	$('body').on( "click", '#cancel-counter-offer', function(ev) {
		ev.preventDefault();
		jQuery.unblockUI();
	});
